...
{
    public class WPFVisualElementTracker<TElement, TNativeElement> : VisualElementTracker where TElement : VisualElement where TNativeElement : FrameworkElement
    {
        bool _disposed;
        ...
        public TNativeElement Control
        {
            ...
            set
            {
                ...
                if (_control != null)
                {
                    _control.IsManipulationEnabled = true;
                    _control.MouseLeftButtonUp += MouseLeftButtonUp;
                    _control.ManipulationDelta += OnManipulationDelta;
                    _control.ManipulationCompleted += OnManipulationCompleted;
                }
        ...
        void HandlePinch(ManipulationDeltaEventArgs e, View view)
        {
            ...
            Point scaleOriginPoint = new Point(e.ManipulationOrigin.X / view.Width, e.ManipulationOrigin.Y / view.Height);

            foreach (PinchGestureRecognizer recognizer in
                view.GestureRecognizers.GetGesturesFor<PinchGestureRecognizer>())
            {
                if (!_isPinching)
                    recognizer.SendPinchStarted(view, scaleOriginPoint);
                else
                    recognizer.SendPinch(view, maxScale, scaleOriginPoint);
            }
            _isPinching = true;
        }
        ...
        {
            double totalX = e.CumulativeManipulation.Translation.X;
            double totalY = e.CumulativeManipulation.Translation.Y;
        ...
        void UpdateTouchFrameReportedEvent()
        {
            ...
            if (!view.GestureRecognizers.GetGesturesFor<PanGestureRecognizer>().Any(g => g.TouchPoints > 1) &&
                !view.GestureRecognizers.GetGesturesFor<PinchGestureRecognizer>().Any())
                return;
